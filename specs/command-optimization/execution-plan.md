...
- .claude/config/integrations.json (per section 6.4.2)

Note: No linear-client.cjs needed - using native GitHub automation (see section 6.1)
```
...
- Section 6.1.6: Spec.json Linear Extension
...
- specs/templates/spec.json (new, 30 lines, include Linear identifier per 6.1.6)
```
...
Note: Linear status updates handled by native GitHub automation (PR opened → In Progress)
```
...
Read specs/command-optimization/synthesis.md section 6.1.6 (Spec.json Extension)
...
- Use unified preview template before execution
- Add Linear integration: create issue via MCP when spec is approved
- Add Linear integration: store issue identifier in spec.json
...
Read specs/command-optimization/synthesis.md section 6.1.4 (Branch Naming Convention)
Read specs/command-optimization/synthesis.md section 6.1.5 (PR Template)
...
- Use PR template with "closes {ISSUE-ID}" for auto-close (per section 6.1.5)
...
Note: Linear status updates handled by native GitHub automation (PR merged → Done)
```
...
```
/design → verify Linear issue created (via MCP)
/implement --task=T001 → open PR → verify Linear auto-updates to "In Progress"
/review
/ship --pr-only → verify Vercel preview deploys
/ship → verify PR has "closes BAS-X" → merge → verify Linear auto-updates to "Done"
```

Note: Status updates happen automatically via Linear's GitHub integration.
...
| ----- | --------------------------------------- | --------------------------------- | ------------------------------------------- |
| 1     | `react-basecamp-foundation`             | `checkpoint-infrastructure`       | checkpoint-manager, schemas, config         |
...
| 5     | `react-basecamp-ship-optimization`      | `ship-incremental-execution`      | --commit-only, Vercel checks, PR template   |
...
## Integration Summary (Native GitHub Automation)

| Integration | Phase | Trigger          | Action                                     |
| ----------- | ----- | ---------------- | ------------------------------------------ |
| Linear      | Setup | One-time         | Configure GitHub integration in Linear     |
| Linear      | 4     | /design approved | Create issue via MCP                       |
| Linear      | -     | PR opened        | Native: Status → In Progress               |
| Linear      | -     | PR merged        | Native: Status → Done (via `closes BAS-X`) |
| Vercel      | 5     | /ship creates PR | Wait for preview deployment                |
| Vercel      | 5     | /ship preview    | Show deployment status                     |

**Key Simplification:** No custom linear-client.cjs needed. Status updates are automatic.
...
1. Phase 1: Foundation (checkpoint-manager, schemas)
